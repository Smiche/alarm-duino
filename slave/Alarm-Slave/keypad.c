...
void KEYPAD_Init()
{
    ...
    M_RowColDirection_B |= (1 << DDB3) | (1 << DDB4); // Configure 2 pins from port b as output
}
...
void KEYPAD_WaitForKeyRelease()
{
	...
	do
	{
		...
		M_ROW=0x0F;           // Pull the ROW lines to low and Column lines high.
		M_ROW_B &= (~((1<<PINB3) | (1<<PINB4)));  // Pull ROW lines on Port B to low
...
int KEYPAD_WaitForKeyPress()
{
	...
	do
	{
		do
		{
            ...
            var_keyPress_u8 = M_COL & 0x0F;     // Read columns
            _delay_ms(1);
            if(loopCounter > 500){
                return 0;
            }
